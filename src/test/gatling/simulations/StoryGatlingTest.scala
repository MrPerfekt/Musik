...
class StoryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Story entity")
        ...
        .repeat(2) {
            ...
            .post("/api/storys")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "title":"SAMPLE_TEXT", "description":"SAMPLE_TEXT", "text":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_story_url")))
            .pause(10)
            .repeat(5) {
